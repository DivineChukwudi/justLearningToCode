...
public class LoginScreen extends JFrame {
    ...
    void login() {
        ...
        try {
            ...
            if (!rs.next()) { rs.close(); ps.close(); chk.close(); UIHelper.err("Invalid username or password."); return; }
            String role = rs.getString("role");
            int pid = rs.getObject("person_id") != null ? rs.getInt("person_id") : -1;
            rs.close(); ps.close(); chk.close();

            DBConnection.conn = role.equals("driver")
                ? DriverManager.getConnection(DBConnection.DB_URL, DBConnection.DRIVER_DBUSER, DBConnection.DRIVER_DBPASS)
                : DriverManager.getConnection(DBConnection.DB_URL, user, pass);
            DBConnection.currentUser = user;
            DBConnection.currentRole = role;
            DBConnection.currentPersonId = pid;
            dispose();
            if (role.equals("driver")) new DriverDashboard();
            else new MainMenu();
        } catch (Exception ex) {
            UIHelper.err("Login failed:\n" + ex.getMessage());
        }
    }
}
