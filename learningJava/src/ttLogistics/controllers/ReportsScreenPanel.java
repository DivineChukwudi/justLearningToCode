...
public class ReportsScreenPanel extends JPanel {

    DefaultTableModel model = new DefaultTableModel();
    JTable table;
    AppFrame appFrame;

    public ReportsScreenPanel(AppFrame appFrame) {
        ...
        JComboBox<String> pidF  = UIHelper.dropdownFromMap(DBConnection.getDrivers());
        ...
        add(UIHelper.bottomBar(null, btnBack, b1, b2, b3, b4, b5, UIHelper.lbl("  Driver:"), pidF, bF, bRefresh),
        ...
        bF.addActionListener(e -> {
            int pid = UIHelper.getSelectedId(pidF);
            if (pid < 0) { UIHelper.err("Select a driver."); return; }
            lastAction[0] = () -> {
                try {
                    ...
                    else UIHelper.err("No driver selected");
                    rs.close(); ps.close();
